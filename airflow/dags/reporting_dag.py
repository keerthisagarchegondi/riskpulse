...
def _generate_daily_summary(**context: Any) -> dict[str, Any]:
    ...
    try:
        ...
        logger.info("Daily summary generated", **{k: v for k, v in summary.items() if k != "top_fraud_merchants" and k != "top_fraud_countries" and k != "alert_summary"})
        return summary
...
def _send_daily_digest(**context: Any) -> dict[str, Any]:
    ...
    if summary.get("alert_summary"):
        ...
        for a in summary["alert_summary"]:
            body_lines.append(
                f"  {a['severity']:<10} | {a['status']:<15} | "
                f"Count: {a['alert_count']:>5}"
            )
        body_lines.append("")
